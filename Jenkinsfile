		...
		stage('Deploy to L2') {
			steps {
				...
				withAnt(installation: 'Ant') {
					bat "ant exportprops"
					bat '''"C:\\Program Files (x86)\\Jenkins\\workspace\\CICDbuild\\scripts\\utils\\prpcServiceUtils.bat export --connPropFile %WORKSPACE%\\scripts\\utils\\%SystemName%_export.properties --artifactsDir %WORKSPACE%" '''
				}
				bat 'echo "4"'
				withAnt(installation: 'Ant') {
				bat 'echo "5"'
